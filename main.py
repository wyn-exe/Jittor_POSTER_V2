...
class RandomErasing:
    ...
    def __call__(self, img):
        ...
        if random.random() > self.p:
            return img
        # img is a numpy array (H, W, C) after ToTensor it's (C, H, W) as numpy
        # jittor ToTensor returns a jittor array; we work on numpy
        import numpy as np
...
def main():
    ...
    if jt.has_cuda:
        jt.flags.use_cuda = 1
        print(f'Using CUDA on GPU: {args.gpu}')
    else:
    ...
    if args.optimizer == 'adamw':
        base_optimizer = jt.optim.AdamW
    elif args.optimizer == 'adam':
        base_optimizer = jt.optim.Adam
    elif args.optimizer == 'sgd':
        base_optimizer = jt.optim.SGD
    else:
        raise ValueError("Optimizer not supported.")

    optimizer = SAM(model.parameters(), base_optimizer, lr=args.lr, rho=0.05, adaptive=False, )
    scheduler = jt.lr_scheduler.ExponentialLR(optimizer, gamma=0.98)
    ...
    if args.evaluate is None:
        ...
        if args.data_type == 'AffectNet-7':
            train_loader = train_dataset.set_attrs(batch_size=args.batch_size, shuffle=True, num_workers=args.workers)
        else:
            train_loader = train_dataset.set_attrs(batch_size=args.batch_size, shuffle=True, num_workers=args.workers)

    test_dataset = ImageFolder(valdir,
                                        transforms.Compose([transforms.Resize((224, 224)),
                                                            transforms.ToTensor(),
                                                            transforms.ImageNormalize(mean=[0.485, 0.456, 0.406],
                                                                                 std=[0.229, 0.224, 0.225]),
    ...
    val_loader = test_dataset.set_attrs(batch_size=args.batch_size, shuffle=False, num_workers=args.workers)

    if args.evaluate is not None:
        if os.path.isfile(args.evaluate):
            print("=> loading checkpoint '{}'".format(args.evaluate))
            import pickle
            with open(args.evaluate, 'rb') as f:
                checkpoint = pickle.load(f)
            best_acc = checkpoint['best_acc']
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        current_learning_rate = optimizer.lr
        ...
        print('Current best accuracy: ', best_acc.item() if hasattr(best_acc, 'item') else best_acc)
        ...
        txt_name = './log/' + time_str + 'log.txt'
        with open(txt_name, 'a') as f:
            f.write('Current best accuracy: ' + str(best_acc.item() if hasattr(best_acc, 'item') else best_acc) + '\n')
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        # SAM first step: compute gradients and perturb weights
        output = model(images)
        ...
        # SAM second step: compute gradients at perturbed point, restore and update
        output = model(images)
        loss = criterion(output, target)
        acc1, _ = accuracy(output, target, topk=(1, 5))
        losses.update(loss.item(), images.shape[0])
...
def validate(val_loader, model, criterion, args):
    ...
    with jt.no_grad():
        for i, (images, target) in enumerate(val_loader):
            ...
            losses.update(loss.item(), images.shape[0])
            top1.update(acc[0], images.shape[0])
...
def save_checkpoint(state, is_best, args):
    ...
    if is_best:
        ...
        with open(args.best_checkpoint_path, 'wb') as f:
            pickle.dump(state, f)
