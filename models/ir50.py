...
class Backbone(Module):
    ...
    def execute(self, x):
        x = jt.nn.interpolate(x, size=(112, 112), mode='bilinear', align_corners=False)
...
def load_pretrained_weights(model, checkpoint):
    ...
    for i, (k, v) in enumerate(state_dict.items()):
        if k.startswith('module.'):
            k = k[7:]
        if k in model_dict and model_dict[k].shape == tuple(v.shape):
            new_state_dict[k] = jt.array(v)
