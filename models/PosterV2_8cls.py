...
from .vit_model_8 import VisionTransformer, PatchEmbed, DropPath, trunc_normal_
...
class feedforward(nn.Module):
    def __init__(self, dim, window_size, mlp_ratio=4., act_layer=nn.GELU, drop=0., drop_path=0., layer_scale=None):
        ...
        if layer_scale is not None and type(layer_scale) in [int, float]:
            ...
            self.gamma1 = nn.Parameter(layer_scale * jt.ones(dim))
            self.gamma2 = nn.Parameter(layer_scale * jt.ones(dim))
        else:
            self.gamma1 = 1.0
            self.gamma2 = 1.0
        self.window_size = window_size
    ...
    def execute(self, attn_windows, shortcut):
...
class pyramid_trans_expr2(nn.Module):
    def __init__(self, img_size=224, num_classes=8, window_size=[28,14,7], num_heads=[2, 4, 8], dims=[64, 128, 256], embed_dim=768):
        ...
        face_landback_checkpoint = load_pth(r'./models/pretrain/mobilefacenet_model_best.pth.tar')
        self.face_landback.load_state_dict(
            {k: jt.array(v) for k, v in face_landback_checkpoint['state_dict'].items()}
        )
        ...
        ir_checkpoint = load_pth(r'./models/pretrain/ir50.pth')
        ...
        dpr = [x.item() for x in jt.linspace(0, 0.5, 5)]
    ...
    def execute(self, x):
        x_face = jt.nn.interpolate(x, size=(112, 112), mode='bilinear', align_corners=False)
