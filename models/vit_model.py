...
def trunc_normal_(tensor, mean=0., std=1., a=-2., b=2.):
    ...
    with jt.no_grad():
        l = norm_cdf(jt.array((a - mean) / std))
        u = norm_cdf(jt.array((b - mean) / std))
        tensor.assign(jt.init.gauss(tensor.shape, 'float32') * std + mean)
        tensor_clamped = jt.clamp(tensor, a, b)
        tensor.assign(tensor_clamped)
...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=14, patch_size=14, in_c=147, num_classes=7,
        ...
        self.pos_embed = jt.zeros(1, in_c + 1, embed_dim)
        ...
        self.eca_block = eca_block()
        dpr = [x.item() for x in jt.linspace(0, drop_path_ratio, depth)]
